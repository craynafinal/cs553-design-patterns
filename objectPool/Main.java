...
public class Main {
	...
	public static void main (String [] arg) {
		...
		for (int i = 0; i < 200; i++) {
			...
			case 0:
				// if even and list is not empty (initially empty),
				// releases the list to the pool after saving its tail to itself
				// so the head returns back to the pool
				if (! list.isEmpty ()) {
				...
				break;
			case 1:
				// if odd, gets a list (Cons<E>) from pool
